//! Configuration instruction for setting up the global program state.
...
/// Accounts required for the configure instruction.
#[derive(Accounts)]
...
impl<'info> Configure<'info> {
    /// Handles the configuration of the program.
    ///
    /// Initializes or updates the global configuration account and ensures
    /// the global vault has sufficient rent.
    ///
    /// # Arguments
    /// * `new_config` - The new configuration to apply
    /// * `config_bump` - The bump seed for the config PDA
