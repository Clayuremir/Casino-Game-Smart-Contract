//! Instruction for creating a new jackpot game round.
...
/// Accounts required for creating a new game.
#[derive(Accounts)]
...
impl<'info> CreateGame<'info> {
    /// Handles the creation of a new game round.
    ///
    /// Validates parameters, requests VRF randomness, and initializes the game state.
    ///
    /// # Arguments
    /// * `force` - Seed for VRF randomness generation
    /// * `round_time` - Duration of the round in seconds
    /// * `min_deposit_amount` - Minimum deposit required to join
