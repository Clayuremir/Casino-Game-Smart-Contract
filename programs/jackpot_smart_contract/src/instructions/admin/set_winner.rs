//! Instruction for setting the winner of a completed game round.
...
/// Accounts required for setting the winner.
#[derive(Accounts)]
...
impl<'info> SetWinner<'info> {
    /// Handles setting the winner for a completed game round.
    ///
    /// Validates that the game is completed, retrieves VRF randomness,
    /// and selects the winner based on weighted random selection.
    ///
    /// # Arguments
