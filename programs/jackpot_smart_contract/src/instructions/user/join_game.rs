//! Instruction for users to join an active game round.
...
/// Accounts required for joining a game.
#[derive(Accounts)]
...
impl<'info> JoinGame<'info> {
    /// Handles a user joining a game round.
    ///
    /// Validates the deposit amount, transfers SOL to the vault,
    /// deducts platform fees, and updates the game state.
    ///
    /// # Arguments
    /// * `round_num` - The round number to join
    /// * `amount` - The amount of SOL to deposit (in lamports)
